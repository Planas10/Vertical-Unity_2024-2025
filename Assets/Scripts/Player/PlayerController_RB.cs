...
public class PlayerController_RB : MonoBehaviour
{
    ...
    private void HorizontalMovement (){
        ...
        //direction = new Vector3(direction.x * _speed, _cc.velocity.y, direction.z * _speed);
    ...
    private void Slide() {
        if (_inputs.actions["Crouch"].WasPressedThisFrame() && !_sliding && !_isCrouched) {
            ...
            _cam.transform.localPosition = _crouchPosition;
            _isCrouched = true;
            Vector3 slideDirection = new Vector3(_cam.transform.forward.x, 0, _cam.transform.forward.z);
            //_rb.AddForce(slideDirection * _slideForce, ForceMode.Impulse);
            this.Invoke("SlideEnd", 0.8f);
        }
    }
